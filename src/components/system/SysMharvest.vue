<template>
  <div id="SysMharvest">
	  <div class = "all-title">
	    <p class="main-font">
	      教师成果管理：共<span>{{ this.count }}</span>项
    ...
    mounted () {
    	this.$nextTick( function() {
				if(this.judgeLogin()){
					...
					this.$ajax.get('/api/getHarvest', {params: Params}).then( res => {
						// console.log(res)
						/**************************************************************************************************jpaper***/
						...
						this.count += (jpaper.length+mpaper.length+patent.length+project.length+subject.length+software.length)
					}).catch( () => {
						alert("出错！请联系管理员")
					});
				}
    ...
    methods:{
        ...
        this.selectionData = val
			...
			handleEdit (row) {
				...
				this.$router.push({ 
					name: 'Addmanual', 
					params: { row: row, harReturnRouter: 'Mharvest' }
				})
			},
      addSelections () {
				// alert("添加成果")
				this.$router.push({ 
					name: 'Addmanual', 
					params: { harReturnRouter: 'Mharvest' }
				})
      },
      delSelections () {
				var _this = this
